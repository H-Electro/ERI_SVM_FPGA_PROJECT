...
try:
    for crop in all_crops:
        logger.info("==============================")
        logger.info("Training model for: %s", crop)
        logger.info("==============================")
        ...
        crop_dir = os.path.join(model_dir, crop_name)
        ...
        with open(os.path.join(crop_dir, "tree.log"), "w") as f:
            f.write(tree_rules)
        ...
        with open(os.path.join(crop_dir, "tree.v"), "w") as f:
        ...
        with open(os.path.join(crop_dir, "tree.py"), "w") as f:
            f.write(python_code)
        ...
        # ------------------------------
        crop_fig_dir = os.path.join(fig_dir, crop_name)
    ...
    logger.info("All crop-specific models generated successfully.")
    if best_crop is not None:
        logger.info("Best crop by R2: %s (R2: %.4f)", best_crop, best_r2)
finally:
    stop_spinner.set()
    spinner_thread.join()
